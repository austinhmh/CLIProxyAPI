...
func headersFromExecResult(resp cliproxyexecutor.Response, execErr error) http.Header {
	...
	if errors.As(execErr, &headerProvider) && headerProvider != nil {
		if errorHeaders := headerProvider.Headers(); len(errorHeaders) > 0 {
			return errorHeaders
		}
	}
	return resp.Headers
...
// hasUnauthorizedAuthFailure reports an explicit terminal unauthorized state.
// Refresh failures with a still-valid token remain scheduled because they keep
// a retry deadline and do not mark the credential unavailable.
...
// error that should neither rotate nor penalize credentials. Credential- and
// model-scoped failures remain eligible for alternate routing.
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time, disableCooling bool) {
	...
	} else if isCloudflareChallengeResultError(resultErr) {
